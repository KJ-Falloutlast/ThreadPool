...
int main()
{
    ...
    //提交任务(可变参模版编程)
    future<int> r1 = pool.submitTask(sum1, 1, 2); //2参数
    future<int> r2 = pool.submitTask(sum2, 1, 2, 3);//3参数
    future<int> r3 = pool.submitTask([](int b, int e)->int { //lambda表达式
        int sum = 0;
        for (int i = b; i <= e; i++)
            sum += i;
        return sum;
